...
class AuthManagerController extends AdminController{
    ...
    public function index(){
        $list = $this->lists('xbAuthGroup',array('module'=>'admin'),'id asc');
    ...
    public function access(){
        $this->updateRules();
        $auth_group = M('xbAuthGroup')->where( array('status'=>array('egt','0'),'module'=>'admin','type'=>AuthGroupModel::TYPE_ADMIN) )
                                    ->getfield('id,id,title,rules');
        $node_list   = $this->returnNodes();
        $map         = array('module'=>'admin','type'=>AuthRuleModel::RULE_MAIN,'status'=>1);
        $main_rules  = M('xbAuthRule')->where($map)->getField('name,id');
    ...
    public function group(){
        $uid            =   I('uid');
        $auth_groups    =   M('xbAuthGroup')->field()->select();
    ...
    public function addToGroup(){
        ...
        if(is_numeric($uid)){
            ...
            if( !M('xbMember')->where(array('uid'=>$uid))->find() ){
                $this->error('用户不存在');
            }
        }
