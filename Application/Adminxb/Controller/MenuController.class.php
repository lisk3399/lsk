...
class MenuController extends AdminController {
    ...
    public function index(){
        $pid  = I('get.pid',0);
        if($pid){
            $data = M('xbMenu')->where("id={$pid}")->field(true)->find();
            $this->assign('data',$data);
        }
        $title      =   trim(I('get.title'));
        $type       =   C('CONFIG_GROUP_LIST');
        $all_menu   =   M('xbMenu')->getField('id,title');
        $map['pid'] =   $pid;
        if($title)
            $map['title'] = array('like',"%{$title}%");
        $list       =   M("xbMenu")->where($map)->field(true)->order('sort asc,id asc')->select();
        ...
        if(IS_POST){
            ...
            if($data){
                ...
                if($id){
                    ...
                    //记录行为
                    action_log('update_menu', 'xbMenu', $id, UID);
            ...
            $menus = M('xbMenu')->field(true)->select();
    ...
    public function edit($id = 0){
        if(IS_POST){
            ...
            $info = array();
            /* 获取数据 */
            $info = M('xbMenu')->field(true)->find($id);
            $menus = M('xbMenu')->field(true)->select();
            $menus = D('Common/Tree')->toFormatTree($menus);
