...
use Adminxb\Model\AuthRuleModel;
...
class AdminController extends Controller {
    ...
    protected function _initialize(){
        ...
        $this->assign('__XB__MENU__', $this->getMenus());
    }
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(empty($menus)){
            ...
            $menus['main']  =   M('xb_menu')->where($where)->order('sort asc')->field('id,title,url')->select();
            ...
            // 查找当前子菜单
            $pid = M('xb_menu')->where("pid !=0 AND url like '%{$controller}/".ACTION_NAME."%'")->getField('pid');
            if($pid){
                // 查找当前主菜单
                $nav =  M('xb_menu')->find($pid);
                if($nav['pid']){
                    $nav    =   M('xb_menu')->find($nav['pid']);
                }
                foreach ($menus['main'] as $key => $item) {
                    // 获取当前主菜单的子菜单项
                    if($item['id'] == $nav['id']){
                        $menus['main'][$key]['class']='current';
                        //生成child树
                        $groups = M('xb_menu')->where(array('group'=>array('neq',''),'pid' =>$item['id']))->distinct(true)->getField("group",true);
                        //获取二级分类的合法url
                        $where          =   array();
                        $where['pid']   =   $item['id'];
                        $where['hide']  =   0;
    ...
    final protected function returnNodes($tree = true){
        ...
        if((int)$tree){
            $list = M('xbMenu')->field('id,pid,title,url,tip,hide')->order('sort asc')->select();
            ...
            $nodes = M('xbMenu')->field('title,url,tip,pid')->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if( stripos($value['url'],MODULE_NAME)!==0 ){
